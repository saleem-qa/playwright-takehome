...
test('open todo', async ({ page }) => {
    ...
    await page.getByText('Write Tests').dblclick();
    ...
    // Type new text
    await editInput.fill('Complete Test');
    await editInput.press('Enter');

    //Adding Duplicate todos
    await todoInput.fill("Tests");
    await todoInput.press("Enter");
    await todoInput.fill("Tests");
    await todoInput.press("Enter");

    //Selecting one todo and marking it as completed
    const testsTodos = page.locator('.todo-list li', { hasText: 'Tests' });
    await testsTodos.nth(1).locator('.toggle').check();
    await page.getByRole('link', { name: 'Completed' }).click();
    ...
    //Checking if duplicate is removed properly
    await expect(toDos).toHaveCount(2);
